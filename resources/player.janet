...
(defn pressed-move-down? [] (or (key-down? :down) (key-down? :s)))
(defn pressed-move-up? [] (or (key-down? :up) (key-down? :w)))
(defn pressed-move-right? [] (or (key-down? :right) (key-down? :d)))
(defn pressed-move-left? [] (or (key-down? :left) (key-down? :a)))
...
(defn draw-player [player]
  ...
  #(draw-circle nx ny 100 [1 0 0 0.5])

  # Draw were the sensors are going to be
  ## (draw-circle (+ nx 8) ny 4 [0 1 1])
  ## (draw-circle nx (- ny 8) 4 [0 1 1])
  ## (draw-circle nx (+ ny 8) 4 [0 1 1])
  ## (draw-circle (- nx 8) ny 4 [0 1 1])
  (if (= [0 0] movement)
    (:draw stand pos 0)
